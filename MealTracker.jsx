...
const AUTHORIZED_CLIENTS = [
  ...
  'Maria Alejandra Gonzales', 'Natalia Samper', 'Alejandro Machado',
];
...
export default function MealTracker() {
  ...
  const inputBarRef2 = useRef(null); // API del ChatInputBar (.prefill/.append/.clear/.focus)
  ...
  // Abrir/cerrar la hoja SOLO toca el DOM (clase CSS), nunca el estado de React.
  // Por eso es instantáneo: no dispara el re-render de este componente gigante,
  // que era lo que congelaba la app y "perdía" la X.
  const closeActionsSheet = useCallback(() => {
    actionsSheetRef.current?.classList.remove('sheet-open');
    document.body.removeAttribute('data-modal-open');
  }, []);

  const openActionsSheet = useCallback(() => {
    actionsSheetRef.current?.classList.add('sheet-open');
  ...
  useEffect(() => {
    const anyOpen = showWellbeingModal || showIngredientsModal || showPlannerModal
      ...
      || editingEntry !== null || pendingFavoriteEntry;
    if (anyOpen) {
      document.body.setAttribute('data-modal-open', '1');
      return;
    }
  ...
  }, [showWellbeingModal, showIngredientsModal, showPlannerModal, showPerformanceModal, showCapabilitiesModal, activeModal, editingEntry, pendingFavoriteEntry]);
  ...
  // Botón "ir al último mensaje" — visibilidad por CSS class (sin setState en
  // cada scroll) para no re-renderizar la app.
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      const visible = (docH - winH - scrollY) > 220;
      if (visible === lastVisible) return;
      lastVisible = visible;
      const el = jumpToLatestRef.current;
  ...
  // (Sync del contenteditable ahora vive dentro de ChatInputBar)

  // Scroll listener: shrink card when scrolled down. Solo dispara setState
  // cuando cruza el umbral (no en cada scroll) para no re-renderizar la app.
  useEffect(() => {
    let last = null;
    const onScroll = () => {
...

{
  ...
  const handleSend = async (textOverride) => {
    const userMsg = (textOverride || inputBarRef2.current?.getText() || '').trim();
    if (!userMsg || loading) return;
    if (!textOverride) inputBarRef2.current?.clear();
    haptic(8);
...
EJEMPLO OUTPUT: {"intent":"log_meal","meal":"desayuno","items":[{"name":"Huevo revuelto","amount":"4 unidades (~200g)","kcal":280,"p":24,"c":2,"g":20},{"name":"Pan integral tostado","amount":"2 unidades (~50g)","kcal":130,"p":5,"c":24,"g":2},{"name":"Manteca","amount":"~10g","kcal":72,"p":0,"c":0,"g":8},{"name":"Café negro","amount":"240ml","kcal":2,"p":0,"c":0,"g":0},{"name":"Creatina","amount":"5g","kcal":0,"p":0,"c":0,"g":0},{"name":"Cacao en polvo","amount":"2 cdas (~10g)","kcal":23,"p":2,"c":6,"g":1}],"quantity_warning":"asumí cantidades estándar; ajusta si difiere"}`;
  ...
  const startVoiceFallback = () => {
    ...
    recognition.onresult = (event) => {
      ...
      inputBarRef2.current?.setFromVoice(txt);
  ...
  const transcribeAudio = async (audioBlob, mimeType) => {
    ...
    try {
      ...
      if (txt) {
        inputBarRef2.current?.append(txt);
  ...
  return (
    <div className="min-h-screen relative" style={{ background: '#F9F7F1', color: TEXT, fontFamily: FONT_UI }}>
      ...
      <style>{`
        ...
        /* La hoja de Herramientas se muestra/oculta SOLO con esta clase (DOM),
           nunca con estado de React: así abrir/cerrar no re-renderiza la app y
           no hay congelamiento. */
        .actions-sheet { display: none; }
        .actions-sheet.sheet-open { display: flex; }
      ...
      <div className="relative max-w-2xl mx-auto px-5 pb-32" style={{ zIndex: 1, paddingTop: cardCompact ? '90px' : '195px' }}>
        ...
        {/* Bottom sheet — actions. Visibilidad controlada por la clase
            `.sheet-open` (DOM), NO por estado de React → abrir/cerrar no
            re-renderiza este componente y por eso es instantáneo. */}
        {(() => {
          return (
          <div
            ref={actionsSheetRef}
            className="actions-sheet fixed inset-0 z-50 items-end justify-center"
            style={{
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2.5">
                    <ActionChipMini icon="🍽️" label="Arma mi día" pastel={ACCENT_PASTEL} color={ACCENT_DARK}
                      onClick={() => { haptic(8); closeActionsSheet(); setShowPlannerModal(true); generatePlan(); }} />
                    <ActionChipMini icon="🔁" label="Repetir comida de ayer" pastel={ACCENT_PASTEL} color={ACCENT_DARK}
                      onClick={() => { haptic(8); repeatYesterday(); closeActionsSheet(); }} />
                    <ActionChipMini icon="⭐" label="Menús favoritos" pastel={C_CARBS_PASTEL} color={C_CARBS}
                      onClick={() => { haptic(8); closeActionsSheet(); setActiveModal('favorites'); }} />
                    <ActionChipMini icon="🛒" label="Mis ingredientes" pastel={C_CARBS_PASTEL} color={C_CARBS}
                      onClick={() => { haptic(8); closeActionsSheet(); setShowIngredientsModal(true); }} />
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2.5">
                    <ActionChipMini icon="📊" label="Mi desempeño" pastel={ACCENT_PASTEL} color={ACCENT_DARK}
                      onClick={() => { haptic(8); closeActionsSheet(); setShowPerformanceModal(true); }} />
                    ...
                    <ActionChipMini icon="🧘" label="Check-in del día" pastel={C_PROTEIN_PASTEL} color={C_PROTEIN}
                      onClick={() => { haptic(8); closeActionsSheet(); setShowWellbeingModal(true); }} />
                    <ActionChipMini icon="📅" label="Calendario" pastel={ACCENT_PASTEL} color={ACCENT}
                      onClick={() => { haptic(8); closeActionsSheet(); setActiveModal('calendar'); }} />
                    <ActionChipMini icon="⚖️" label="Ayuda con proporciones" pastel={C_PROTEIN_PASTEL} color={C_PROTEIN}
                      onClick={() => { haptic(8); closeActionsSheet(); requestAnimationFrame(() => requestAnimationFrame(() => { inputBarRef2.current?.prefill('Ayúdame con proporciones, tengo: '); inputBarRef2.current?.focus(); })); }} />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2.5">
                    ...
                    <ActionChipMini icon="❓" label="¿Qué puedo hacer?" pastel={ACCENT_PASTEL} color={ACCENT_DARK}
                      onClick={() => { haptic(8); closeActionsSheet(); setShowCapabilitiesModal(true); }} />
                    <ActionChipMini icon="🔄" label="Reiniciar día" pastel="#E5E2D5" color={TEXT_MUTED}
                      onClick={() => { haptic(8); closeActionsSheet(); setActiveModal('reset'); }} />
                  </div>
                </div>
              </div>
            </div>
      ...
      {/* Jump-to-latest floating arrow — siempre montado; visibilidad por DOM */}
      <button
        ref={jumpToLatestRef}
        onClick={() => { haptic(6); window.scrollTo({ top: document.documentElement.scrollHeight, behavior: 'smooth' }); }}
        className="fixed z-40 rounded-full transition active:scale-95 items-center justify-center"
        style={{
          display: 'none',
      ...
      <div ref={inputBarRef} className="fixed bottom-0 left-0 right-0 px-4 pb-5 pt-6 z-40" style={{
        ...
        <ChatInputBar
          ...
          onFocusInput={closeActionsSheet}
          onSend={(text, opts) => { if (opts?.fromVoice) voiceInputRef.current = true; handleSend(text); }}
        />
      </div>
...
// ─────────────────────────────────────────────────────────────────────────
// ChatInputBar — el input del chat con su PROPIO estado interno.
// Antes el `input` vivía en MealTracker, así que cada keystroke re-renderizaba
// las 6000 líneas y la app se congelaba en cada letra. Aislándolo aquí, el
// padre solo se entera cuando el usuario envía. El padre lo controla via ref
// (.prefill / .append / .clear / .focus) para casos como voz o atajos.
// ─────────────────────────────────────────────────────────────────────────
const NO_PREDICTION_HINT = /desayuno|almuerzo|cena|snack|reiniciar|cambiar|resumen|semanal|calendario|favoritos|proporciones|agua|cuántas|cuanto|cuánto/;

const ChatInputBar = memo(forwardRef(function ChatInputBar({
  onSend, onStartVoice, onStopVoice, onFocusInput,
  recording, transcribing, loading, predictedMeal,
}, ref) {
  const [text, setText] = useState('');
  const elRef = useRef(null);
  const voiceFlagRef = useRef(false);

  const setBoth = useCallback((v) => {
    setText(v);
    const el = elRef.current;
    if (!el) return;
    if (!v) { if (el.innerHTML !== '') el.innerHTML = ''; return; }
    if ((el.textContent || '') !== v) {
      el.textContent = v;
  ...
  useImperativeHandle(ref, () => ({
    prefill: (v) => { setBoth(v || ''); voiceFlagRef.current = false; },
    append: (v) => { if (!v) return; setBoth((text && text.trim() ? text.trim() + ' ' : '') + v); voiceFlagRef.current = true; },
    setFromVoice: (v) => { setBoth(v || ''); voiceFlagRef.current = !!v; },
    clear: () => setBoth(''),
    focus: () => elRef.current?.focus(),
    consumeVoiceFlag: () => { const f = voiceFlagRef.current; voiceFlagRef.current = false; return f; },
    getText: () => text,
  }), [setBoth, text]);
  ...
  const send = () => {
    const t = (text || '').trim();
    if (!t) return;
    const fromVoice = voiceFlagRef.current;
  ...
  const showHint = trimmed && !NO_PREDICTION_HINT.test(trimmed.toLowerCase());
  ...
  return (
    <div className="max-w-2xl mx-auto">
      {recording && (
        <div className="flex items-center justify-center gap-1 mb-2 h-6">
          {[0,1,2,3,4,5,6,7].map(i => (
            <div key={i} style={{ width:'3px', height:'100%', background:C_PROTEIN, borderRadius:'2px',
              animation:`wave 0.9s ease-in-out ${i*0.08}s infinite`, transformOrigin:'center' }} />
          ))}
        </div>
      )}
      {showHint && (
      ...
      <div className="flex items-center gap-2 p-2 rounded-2xl" style={{
        ...
        />
        <button type="button" onClick={recording ? onStopVoice : onStartVoice} disabled={transcribing}
          className="rounded-xl transition active:scale-[0.95] disabled:opacity-60 shrink-0 flex items-center justify-center"
          style={{ width:'46px', height:'46px', background: recording ? C_PROTEIN : '#1F1F1F', color:'#fff',
            boxShadow: recording ? `0 0 0 3px ${C_PROTEIN}30` : '0 2px 8px rgba(0,0,0,0.18)', transition:'background 0.2s, box-shadow 0.2s' }}
          title={recording ? 'Detener dictado' : transcribing ? 'Transcribiendo…' : 'Dictar por voz'}>
          {transcribing ? <Loader2 size={20} strokeWidth={2} className="animate-spin" />
